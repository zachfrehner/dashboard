...
export function RideDetailPage() {
  ...
  return (
    <Stack spacing={3}>
      <PageHeader title={data?.name ?? 'Ride Detail'} subtitle={data ? new Date(data.startedAt).toLocaleString() : undefined} />
      {data && (
        <>
          <MetricGrid
            metrics={[
              ...
              { title: 'Avg Power', value: `${data.summary.averagePowerWatts} W` },
              { title: 'Heart Rate', value: `${data.summary.averageHeartRateBpm} bpm` },
            ]}
          />
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <DashboardCard title="BurnMetrix Calculations" detail={data.burnMetrixSummary} />
            </Grid>
            <Grid item xs={12} md={6}>
              <DashboardCard title="Notes" detail={data.notes} />
            </Grid>
          </Grid>
        </>
      )}
